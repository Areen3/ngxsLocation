...
export function registerComponent<T>(vehicle: VehicleEnum): (target: Type<any>) => void {
...
export function getRegisterComponent<T>(vehicle: VehicleEnum): Type<T> {
  if (!MapDecorator.has(vehicle)) {
    throw new Error(
      `You should first decorate component: ${vehicle} using decorator : @${registerComponent.name}`
    );
  }
