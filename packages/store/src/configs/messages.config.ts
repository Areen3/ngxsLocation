...
export function throwStateNotFoundError(path: string): never {
  throw new Error(`State in localization'${path}' doesn't exist`);
}

export function throwToManyStatgeFoundError(parentName: string): never {
  throw new Error(`State with name '${parentName}' found in many location`);
}

export function throwMissingSelectorDecoratorError(selectorName: string): never {
  throw new Error(`Method ${selectorName} is not decorated`);
}
